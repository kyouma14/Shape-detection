...
cap=cv2.VideoCapture(r'C:\Users\soham\Downloads\test2.mp4')
while True:
    ret,frame=cap.read()
...
#red
    ...
    #blue
    low_blue=np.array([54,80,2],np.uint8)
    ...
    if len(centroidsR)>1:
        frame=cv2.line(frame,(centroidsR[0]),(centroidsR[1]),(0,0,255),5)
    if len(centroidsG)>1:
        frame=cv2.line(frame,(centroidsG[0]),(centroidsG[1]),(0,255,0),5)
    if len(centroidsB)>1:
        frame=cv2.line(frame,(centroidsB[0]),(centroidsB[1]),(255,0,0),5)
